...
{
    if ( !m_ObjectList ) {
        m_ObjectList.Reset(new TObjectList());
        return;
    }
    (*m_ObjectList).Reset();
}
...
BEGIN_NAMED_BASE_CLASS_INFO("ObjectSelector", CObjectSelector)
{
    SET_CLASS_MODULE("linkout");
    ADD_NAMED_STD_MEMBER("Database", m_Database)->SetSetFlag(MEMBER_PTR(m_set_State[0]))->SetNoPrefix();
    ADD_NAMED_REF_MEMBER("ObjectList", m_ObjectList, CObjectList)->SetNoPrefix();
    info->RandomOrder();
}
END_CLASS_INFO
