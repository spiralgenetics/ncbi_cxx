...
void CTMgr_DisplayTrackRequest_Base::ResetGenome_contexts(void)
{
    m_Genome_contexts.clear();
    m_set_State[0] &= ~0xc;
}
...
BEGIN_NAMED_BASE_CLASS_INFO("TMgr-DisplayTrackRequest", CTMgr_DisplayTrackRequest)
{
    ...
    ADD_NAMED_MEMBER("genome-contexts", m_Genome_contexts, STL_list, (STL_CRef, (CLASS, (CTMgr_GenomeContext))))->SetOptional()->SetSetFlag(MEMBER_PTR(m_set_State[0]));
    ADD_NAMED_MEMBER("attrs", m_Attrs, STL_list, (STL_CRef, (CLASS, (CTMgr_AttrSpec))))->SetOptional()->SetSetFlag(MEMBER_PTR(m_set_State[0]));
    info->RandomOrder();
}
END_CLASS_INFO
