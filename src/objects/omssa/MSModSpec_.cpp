...
BEGIN_NAMED_BASE_CLASS_INFO("MSModSpec", CMSModSpec)
{
    ...
    ADD_NAMED_MEMBER("residues", m_Residues, STL_list, (STD, (string)))->SetOptional()->SetSetFlag(MEMBER_PTR(m_set_State[0]));
    ADD_NAMED_REF_MEMBER("neutralloss", m_Neutralloss, CMSMassSet)->SetOptional();
    ADD_NAMED_STD_MEMBER("unimod", m_Unimod)->SetOptional()->SetSetFlag(MEMBER_PTR(m_set_State[0]));
    ADD_NAMED_STD_MEMBER("psi-ms", m_Psi_ms)->SetOptional()->SetSetFlag(MEMBER_PTR(m_set_State[0]));
    info->RandomOrder();
}
END_CLASS_INFO
